...
(extend-protocol Measured
  Number (magnitude [n] (rationalize n)))
...
;; approximate arithmetic — never exact.
(def ^:private default-mc
  "commensura's fallback context for approximate arithmetic when the caller has
  not bound `*math-context*`: 34-significant-digit DECIMAL128."
...
(defn qmul [x y]
  (let [formula' (combine-terms (concat (formula x) (formula y)))]
    (if (or (approx? x) (approx? y))
      ...
      (->PreciseQuantity (* (rationalize (magnitude x))
                            (rationalize (magnitude y)))
                         formula'))))
...
(defn qpow [x n]
  (let [formula' (formula-pow (formula x) n)]
    (if (approx? x)
      ...
      (->PreciseQuantity (rationalize (npow (magnitude x) n)) formula'))))
...
(defn to
  ...
  (if (or (approx? q) (approx? target))
    ...
    (->PreciseQuantity (rationalize (magnitude q)) (formula target))))
...
(defn ratio
  ...
  (if (or (approx? q) (approx? target))
    ...
    (->PreciseQuantity (/ (rationalize (magnitude q))
                          (rationalize (magnitude target))) [])))
